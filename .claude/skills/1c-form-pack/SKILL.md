...
```cmd
python .1c-tools/pack_form_modules.py <каталог> [--no-backup] [--verbose]
```
...
```cmd
# Упаковать (с backup)
python .1c-tools/pack_form_modules.py src

# Упаковать без backup
python .1c-tools/pack_form_modules.py src --no-backup

# Подробный вывод
python .1c-tools/pack_form_modules.py src --verbose
```
...
3. **Упаковать: `python .1c-tools/pack_form_modules.py src`**
