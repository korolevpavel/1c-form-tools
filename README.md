...
неисправный venv (включая отсутствующий интерпретатор или оборванную символическую
ссылку) не подменяются другим окружением. На Windows для `PYTHON`
...
## Проверки запуска и Git-хука

Имя модуля в хуке сопоставляется без учёта регистра (`Module.bsl`, `module.bsl`,
`MODULE.BSL`). Хук читает рабочую копию: включите все изменения пары Module.bsl / Form.bin
...
контейнеры без прикладных данных: проверяются выбор Python, диагностика,
проектный venv без активации, частичный staging и настоящий Git-коммит через хук.
Тесты создают Module.bsl напрямую и не зависят от отдельного исправления извлечения CRLF.
Проверка открытия формы в платформе 1С выполняется отдельно и не заменяется этим CI.
